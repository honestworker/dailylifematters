...
$mail_user_mail = "info@host.com";
$mail_user_name = "Name";

$mail->Username = $mail_user_mail;    
$mail->Password = "pass";

$mail->From = $mail_user_mail;
...
$emailTO[] = array( 'email' => 'gmail@gmail.com', 'name' => '' );
...
//$formEmail = 'noreply@host.com';

### Success Messages
$msg_success = "We have <strong>successfully</strong> received your message. We'll get back to you soon.";

if( $_SERVER['REQUEST_METHOD'] == 'POST') {
	if (isset($_POST["email"]) && $_POST["email"] != '' && isset($_POST["name"]) && $_POST["name"] != '' && isset($_POST["subject"]) && $_POST["subject"] != '') {
		### Form Fields
		$cf_email = $_POST["email"];
		$cf_name = $_POST["name"];
		$cf_subject = $_POST["subject"];
		$cf_message = isset($_POST["message"]) ? $_POST["message"] : '';
		...
		if ($honeypot == '' && !(empty($emailTO))) {
			...
			$bodymsg .= isset($cf_subject) ? "Subject: $cf_subject<br><br>" : '';
			$bodymsg .= isset($cf_message) ? "Message: $cf_message<br><br>" : '';
			$bodymsg .= $_SERVER['HTTP_REFERER'] ? '<br><br>This email was sent from: ' . $_SERVER['HTTP_REFERER'] : '';
